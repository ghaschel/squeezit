...
Source: [Commitlint's official agent skill](https://commitlint.js.org/guides/ai-agents.html),
adapted only to use this Bun-managed repository's local executable.
...
This repository configures `block-no-verify` as an agent pre-tool hook. It
blocks Git commands and GitHub MCP write tools that would bypass the local
Husky/commitlint hook chain. It is enforcement, not a replacement for the
validation command in this skill: write a valid message first, validate it,
and let the normal Git hooks run.
...
Closes #123" | bunx --no-install commitlint
```
